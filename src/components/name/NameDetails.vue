...
<template>
    <div id="ContentContainer" class="register-name-panel">
        <div class="domain-name-frame">
            <div class="domain-name-frame-title">{{ domainName }}</div>
            <div class="domain-name-frame-toolbar">
                <UnitButton :caption="$t('singleName.tabs.register')" @onClick="onNameRegisterButtonClick"
                    v-if="!isSubdomain" :enable="true"></UnitButton>
        ...
        <div v-if="!isSubdomain" style="width: 100%;">


            <div class="detail-panel" v-if="available">
                <DetailItemReadonly :title="$t('c.parent')" :content="parent"></DetailItemReadonly>

                <DetailItemReadonly :title="$t('c.registrant')" :content="$t('singleName.messages.noowner')">
                </DetailItemReadonly>

                <DetailItemReadonly :title="$t('c.Controller')" :content="$t('singleName.messages.noowner')">
                </DetailItemReadonly>

                <DetailItemReadonly :title="$t('c.Resolver')" :content="$t('singleName.messages.noresolver')">
                </DetailItemReadonly>
            </div>
            <div class="detail-panel" v-else>
                ...
                <DetailAddressItem :title="$t('c.registrant')" :content="registrant" :buttonCaption="$t('c.transfer')"
                    :enable="enableRegistrantEdit" @onOkButtonClick="onRegistrantButtonClick"></DetailAddressItem>

                <DetailAddressItem v-if="isRegistrant" :title="$t('c.Controller')" :content="owner"
                    :buttonCaption="$t('c.set')" :enable="enableControllerEdit"
                    @onOkButtonClick="onControllerSetButtonClick"></DetailAddressItem>

                <DetailAddressItem v-else :title="$t('c.Controller')" :content="owner" :buttonCaption="$t('c.transfer')"
                    :enable="enableControllerEdit" @onOkButtonClick="onControllerTransferButtonClick">
                </DetailAddressItem>

                <DetailExpiration :domainName="domainName" :expiryTime="expiryTime"
...
export default {
    name: "NameDetails",
    components: {
        DetailExpiration, DetailAddressItem, DetailItemReadonly, DetailContentItem
    },
    props: {
        domainName: {
            type: String,
    ...
    computed: {
        ...
        isExpired() {
            if (this.domainEntry != null)
                return this.domainEntry.expiryTime < new Date()
            return true
        },
        //账户是否是域名的拥有者
        isOwner() {
    ...
    methods: {
        ...
        async init() {
            if (this.isSubdomain)
                this.initSubdomain()
            else this.initDomain()
        },

        async initDomain() {

            var options = { target: document.querySelector('#RegisterContainer') }
            const loadingInstance = ElLoading.service(options)
...
.detail-base-info-container {
    display: flex;
    width: 100%;
}

.detail-base-info-left {
    font: 1em sans-serif;
    width: 200px;
    height: 2em;
    text-align: right;
...
.details-records-panel {
    border-radius: 5px 5px 0 0;
    border: 1px solid #bbbbbb;
}

.details-records-caption {
    border-radius: 5px 5px 0 0;
    background: linear-gradient(160deg, #e6e7e7 0%, #cdcdce 100%);
    font: 0.8em sans-serif;
    padding: 0.2em;
}

.details-records-left {
    font: 1em sans-serif;
    width: 250px;
    text-align: right;
}

.details-records-right {
    font: 1em sans-serif;
