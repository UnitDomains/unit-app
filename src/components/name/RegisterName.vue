...
<template>
    <div id="RegisterContainer" class="register-name-panel">
        <div class="domain-name-frame">
            ...
            <div class="domain-name-frame-toolbar">
                <UnitButton :caption="$t('singleName.tabs.register')" @onClick="onNameRegisterButtonClick"
                    :enable="true" type="primary"></UnitButton>
        ...
        <div v-if="domainNameAlreadyRegistered == 1">
            <div class="already-registered-title">{{ $t('singleName.messages.alreadyregistered') }}</div>
        </div>
        <div v-else-if="domainNameAlreadyRegistered == 2">
            <div class="register-duration" v-show="registerDurationVisible">
                <RegisterDuration :domainName="domainName" :years="years" @onDurationChange="onDurationChange"
                    @onDurationBeginChange="onDurationBeginChange"></RegisterDuration>
            </div>
            ...
            <div class="operation-panel">
                <UnitButton :caption="$t('register.buttons.request')" @onClick="onRequestRegistrar"
                    :enable="!accountBalanceInsufficientVisible && requestRegistrarButtonEnable"
                    v-show="requestRegistrarVisible" type="primary"></UnitButton>

                <UnitButton :caption="$t('register.buttons.register')" @onClick="onRegistrar"
                    :enable="!accountBalanceInsufficientVisible && registerButtonEnable" type="primary"
                    v-show="registrarVisible"></UnitButton>

                <UnitButton :caption="$t('register.buttons.setreverserecord')" @onClick="onSetReverseRecord"
                    type="primary" :enable="!accountBalanceInsufficientVisible" v-show="reverseVisible"></UnitButton>
            </div>
        </div>
    </div>
</template>
...
export default {
    name: "RegisterName",
    components: {
        Step, ProgressBar, ProgressText, RegisterDuration
    },
    props: {
        domainName: {
            type: String,
    ...
    methods: {
        ...
        waitOneMinuteTimerHelper() {
            ...
            if (savedStep) {
                ...
                if (secondsPassed >= 60 && this.waitOneMinuteId != null) {//wait 60 second(a minute)
        ...
        async initProgressStore() {
            ...
            if (savedStep.commitmentExpirationDate && moment(savedStep.commitmentExpirationDate).isSameOrBefore(now)) {
        ...
        }, onDurationBeginChange() { this.requestRegistrarButtonEnable = false },
        ...
        async onDurationChange(years, rentPrice, gasPrice) {
            ...
            if (totalFees.gt(accountBalance)) { this.accountBalanceInsufficientVisible = true }
            else
        ...
        * 请求注册
        ...
        async onRequestRegistrar() {
            try {
                ...
                const block = await getBlock()
                let commitmentExpirationDate = moment(block.timestamp * 1000).add(await registrar.getMaximumCommitmentAge(), 'second')
                ...
                if (error.message.includes('User denied transaction signature.') ||//Metamask
