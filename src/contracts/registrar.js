...
export default class Registrar {
    ...
    async getAvailable(domainName) {
        ...
        return await this.permanentRegistrarController.available(label, baseNodeIndex)
    }
    ...
    async reclaim(domainName, address, overrides = {}) {
        ...
        try {
            ...
            if (parseInt(networkId) > 1000) {
                /* if private network */
                const gas = await Registrar.estimateGas.reclaim(label, address, baseNodeIndex)
    ...
    async getRentPrice(domainName, duration) {
        ...
        let price = await permanentRegistrarController.rentPrice(name, duration, baseNodeIndex)
        return price
    }
    ...
    async getRegisterPrice(domainName, duration) {
        ...
        var baseNodeIndex = getDomainIndex(domainName)
        const permanentRegistrarController = this.permanentRegistrarController
        let price = await permanentRegistrarController.registerPrice(name, duration, baseNodeIndex)
        return price
    }
    ...
    async getRentPrices(domainsName, duration) {
        ...
        const pricesArray = await Promise.all(
            domainsName.map(domainName => {
                ...
                var domainNameSuffix = getDomainSuffix(domainName)
                var baseNodeIndex = getDomainIndex(domainName)
                return this.getRentPrice(label, duration).add(this.getRegisterPrice(name, duration))
            })
        )
    ...
    async makeCommitment(domainName, owner, secret = '') {
        ...
        if (parseInt(resolverAddr, 16) === 0) {
            return permanentRegistrarController.makeCommitment(name, owner, secret, baseNodeIndex)
        } else {
...
*/
    ...
    async renew(domainName, duration) {
        ...
        const gasLimit = await this.estimateGasLimit(() => {
            return permanentRegistrarController.estimateGas.renew(name, duration, baseNodeIndex, {
                value: priceWithBuffer
            })
        })
